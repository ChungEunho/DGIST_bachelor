...
if __name__ == "__main__":
    ...
    mask_clean = morphologyf(morphologyf(morphologyf(morphologyf(mask,0),1),1),0).astype(bool)
    save_image(mask_clean, "orange2_mask_clean.png")

    recon = mask_clean.astype(np.uint8)

    for iteration, radius in enumerate([40, 30, 20], 1):

        # 침식하여 seeds 생성
        se = disk(radius=radius)
        seeds = erode(recon.astype(bool), se)
        save_image(seeds, f"orange2_iter{iteration}_seeds.png")
        ...
        save_image(labels / labels.max() * 255, f"orange2_iter{iteration}_seed_labels.png")

        # Morphological Reconstruction (seed 확장)
        recon = labels.copy()
        ...
        visualize_colored(recon, f"orange2_iter{iteration}_recon_color.png")

    # 최종 결과 원형 객체 선별 및 크기 필터링
    ...
    save_image(binary_final, "orange2_final_filtered_binary.png")
    visualize_colored(final_map, "orange2_final_filtered_overlay.png")

    np.save("orange2_final_map.npy", final_map)
