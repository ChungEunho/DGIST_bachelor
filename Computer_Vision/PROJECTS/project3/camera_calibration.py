...
images = [f for f in os.listdir('captured') if f.endswith('.jpeg')]
for fname in images:
...
if len(objpoints) > 0 and len(imgpoints) > 0:
    ...
    camera_matrix_filename = f"parameters/camera_matrix.txt"
    dist_coeffs_filename = f"parameters/dist_coeffs.txt"
    ...
    for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
        ...
        extrinsics_filename = f"parameters/extrinsics_{i}.txt"
